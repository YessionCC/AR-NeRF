...
class NGP(nn.Module):
    def __init__(self, scale, rgb_act='Sigmoid', use_raw_HDR = False):
        super().__init__()

        self.rgb_act = rgb_act
        self.use_raw_HDR = use_raw_HDR
        ...
        if self.rgb_act == 'None' and not self.use_raw_HDR: # rgb_net output is log-radiance
    ...
    def forward(self, x, d, **kwargs):
        ...
        # NOTE: RELU should never be used in rgb output training!!!
        # it will cause unconverge !!!!
        # if need clamp range, use sigmoid, otherwise, 
        # no directly output is still better than RELU !!!
        if self.use_raw_HDR:
            ...
            if kwargs.get('output_radiance', False): # output HDR map
                rgbs = torch.clamp(rgbs, 0, 20) # avoid float overflow # Not sure if RIGHT !!
